...
(async () => {
  ...
  async function deriveSubKey(masterRaw, idx) {
    const ikm = await crypto.subtle.importKey(
      'raw', masterRaw, 'HKDF', false, ['deriveBits']
    );
    const info = new TextEncoder().encode(`chunk-${idx}`);
    const bits = await crypto.subtle.deriveBits(
      { name: 'HKDF', hash: 'SHA-256', salt: CHUNK_SALT, info },
      ikm, 256
    );
    return crypto.subtle.importKey(
      'raw', bits, { name: 'AES-CTR', length: 256 }, false, ['decrypt']
    );
  }

  async function deriveKey() {
    // 1) Lấy pubkey/ signature và verify như cũ
    ...
    // 5) Import thành CryptoKey và trả về cả hai
  ...
  async function play(file, btn) {
    ...
    try {
      const { aesKey, keyBits } = await deriveKey();
      ...
      while (true) {
        ...
        while (buffer.byteLength >= 8 + CHUNK_SIZE) {
          ...
          const subKey = await deriveSubKey(keyBits, Number(idx));
        ...
        if (done) {
          if (buffer.byteLength >= 8) {
            ...
            // ⬇️ derive sub-key cho chunk cuối
            const subKey = await deriveSubKey(keyBits, Number(idx));
            const plainBuf = await crypto.subtle.decrypt(
              { name: 'AES-CTR', counter: iv, length: 128 },
              subKey,
